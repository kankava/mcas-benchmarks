...
void benchmark_bst() {
  ...
  {
    lockfree_mcas::BinarySearchTree<int> l1;
    /* prefill list with 1024 elements */
    for(int i = 0; i < DATA_PREFILL; i++) {
      l1.insert(uniform_dist(engine));
    }
    benchmark(6, u8"lock-free mcas binary search tree lookup", [&l1](int random){
      bst_lookup(l1, random);
    });
    benchmark(6, u8"lock-free mcas binary search tree update", [&l1](int random){
      bst_update(l1, random);
    });
  }

  {
    ...
    benchmark(6, u8"lock-free mcas binary search tree mixed", [&l1](int random){
      bst_mixed(l1, random);
    });
  }
}
