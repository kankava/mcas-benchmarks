...
class Buffer {
 ...
 public:
  ...
  Buffer<T> *next_buffer() { return next; }

  long size() const { return static_cast<long>(1 << log_size); }

  T get(long i) const { return segment[i % size()]; }
  ...
  Buffer<T> *resize(long b, long t, int delta) {
    ...
    for (auto i = t; i < b; ++i) buffer->put(i, get(i));
    next = buffer;
    return buffer;
  }
};
...
class Reclaimer {
 ...
 public:
  ...
  buffer_tls *get_id_list() { return id_list.load(std::memory_order_relaxed); }
...
class Worker {
 ...
 public:
  ...
  std::experimental::optional<T> pop() { return deque->pop_bottom(); }
};
...
class Stealer {
 ...
 public:
  explicit Stealer(std::shared_ptr<Deque<T>> d)
      : deque(d), buffer_data(deque->reclaimer.register_thread()) {}

  // Copy constructor.
  //
  // Used whenever a new stealer thread is created.
  Stealer(const Stealer<T> &s)
      : deque(s.deque), buffer_data(deque->reclaimer.register_thread()) {}
  ...
  // thread.
  Stealer(Stealer<T> &&s)
      : deque(std::move(s.deque)), buffer_data(s.buffer_data) {}
